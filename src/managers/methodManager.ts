...
export default class methodManager {
  ...
  public static consolidateMethods(
    methods: IMethod[],
    methodsDirUri: vscode.Uri
  ): methodFileError[] {
    ...
    if (methodsDirStats.isDirectory()) {
        ...
        if (!Files.exists(methodUri)) {
          methodsErrors.push(
            new methodFileError(
              ...
              null,
              method,
              MethodErrorReason.NotExistFile
            )
          );
        } else {
          let methodDirStats = Files.readFileStats(methodUri);
          if (methodDirStats.isFile()) {
            let methodFileContent = Files.GetContent(methodUri);
            ...
            if (methodFileContent.hasOwnProperty("code")) {
              if (method.code !== methodFileContent.code) {
                ...
                console.log(
                  `Different code between json and file for ${methodUri}`
                );
                methodsErrors.push(
                  new methodFileError(
                    methodUri,
            ...
            if (methodFileContent.hasOwnProperty("signature")) {
              if (method.signature !== methodFileContent.signature) {
                // error, reference json and file content is different
                console.log(
                  `Different signature between json and file for ${methodUri}`
                );
                methodsErrors.push(
                  new methodFileError(
                    methodUri,
      ...
      methodsDir.forEach(async methodFile => {
        ...
        let methodUri = vscode.Uri.file(
          methodsDirUri.fsPath + "\\" + methodFile
        );
        let methodDirStats = Files.readFileStats(methodUri);
        if (methodDirStats.isFile() && methodFile.indexOf("method_") > -1) {
          let index: number = Number.parseInt(
            methodFile.replace("method_", "").replace(".lua", "")
          );
          ...
          if (isNullOrUndefined(methodFromJson)) {
            ...
            methodsErrors.push(
              new methodFileError(
                ...
                methodFileContent,
                null,
                MethodErrorReason.NotExistJson
              )
            );
          }
        }
  ...
  public static GetMethodFileUri(root: vscode.Uri, index: number): vscode.Uri {
    let handlerFilename: string = `${methodManager.GetMethodFilename(
      index
    )}.lua`;
