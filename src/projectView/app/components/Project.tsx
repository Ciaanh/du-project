...
import { SlotIndexes } from "../interfaces/slotIndexes";
import { OverviewMode } from "../interfaces/enums";
...
> {
  ...
  private shouldHandleErrors(): boolean {
    if (this.state.duProject.overviewMode === OverviewMode.Project) {
      return true;
    }
    return false;
  }
      ...
      <ul className="">
        {handlers && handlers.length > 0
          ? handlers.map((handler, handlerIndex) => {
              let argList =
                handler.filter.args && handler.filter.args.length > 0
                  ? handler.filter.args
                      .map(arg => {
                        return arg.value;
                      })
                ...
                >
                  <a
                    className={
                      ...
                      this.state.currentHandler.key === handler.key
                        ? " active"
                        : "")
                    }
                    href="#"
  ...
  private renderHandler(handler: IHandler) {
    return (
      <div className="">
        <a className="editHandler" onClick={() => this.editHandler(handler)}>
          Edit this code
        </a>
    ...
    return (
      <ul className="nav flex-column">
        {this.state.duProject
          ? SlotIndexes.indexes.map(slotIndex => {
              ...
              return (
                <li key={slotIndex} className="nav-item">
      ...
      <React.Fragment>
        <nav className="navbar navbar-dark fixed-top bg-dark flex-md-nowrap p-0 shadow">
          <a className="navbar-brand col-sm-3 col-md-2 mr-0" href="#">
            DU Project - json to lua overview
          </a>
        ...
        <div className="container-fluid">
          <div className="row">
            <nav className="col-2 sidebar">
              <div className="sidebar-sticky">{this.renderSlotList()}</div>
            </nav>
