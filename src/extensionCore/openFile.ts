...
export default class ProjectPicker {
  ...
  private showPickList(
    itemList: vscode.QuickPickItem[] | Thenable<vscode.QuickPickItem[]>,
    callback: Function
  ) {
  ...
  private listDirectoryFiles(): Thenable<vscode.QuickPickItem[]> {
    ...
    console.log("listDirectoryFiles");

    return results.then((items) => {
      console.log("items =>");
      console.log(items);
      // display message if items is empty => no file found
      return items.map<vscode.QuickPickItem>((value) => {
        let valuePath = value.path.split("/");
        let projectJsonName = valuePath[valuePath.length - 1].replace(
          ".json",
          ""
        );
  ...
  private async listDirectoryFolders(): Promise<vscode.QuickPickItem[]> {
    ...
    console.log("listDirectoryFolders");
    console.log(workspaces);
    if (workspaces) {
      const quickPickItemByWorkspace = await Promise.all(
        workspaces.map(async (workspaceFolder) => {
          const content = await this.getDuFolders(workspaceFolder.uri);

          const quickPickItems: vscode.QuickPickItem[] = content.map(
            (folder) => {
              let folderUri = vscode.Uri.file(
                workspaceFolder.uri.fsPath + "\\" + folder
              );
              let documentpath = folderUri.path.split("/");
              let documentName = documentpath[documentpath.length - 1].replace(
                "du_",
                ""
              );
  ...
  private async getDuFolders(folderUri: vscode.Uri): Promise<Array<string>> {
    ...
    const refinedElements = await Promise.all(
